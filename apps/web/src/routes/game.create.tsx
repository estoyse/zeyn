...
function CreateGamePage() {
  ...
  const canCreate = name.length >= 3 && selectedSubjectIds.length >= 5 && selectedSubjectIds.length <= 10;
      ...
      <div className="mx-auto max-w-6xl space-y-8">
        <header className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold tracking-tight">Setup Your Arena</h1>
              <p className="text-muted-foreground italic">Configure the battlefield before deploying.</p>
            </div>
          </div>
        </header>
            ...
            <Card className="glass-card overflow-hidden">
              ...
              <CardContent className="p-6 space-y-6">
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">Minimum 3 characters. Visible to players.</p>
                </div>
                  ...
                  <div className="space-y-4 pt-8">
                    <div className="flex items-center justify-between p-3 rounded-lg border bg-muted/20">
                      <div className="space-y-0.5">
                        <Label className="flex items-center gap-2">
                          {isPublic ? <Globe className="size-4 text-primary" /> : <Lock className="size-4 text-destructive" />}
                ...
                <AnimatePresence>
                  {!isPublic && (
                    ...
                    >
                      <Label className="flex items-center gap-2">
                        <Shield className="size-4 text-primary" /> Room Password (Optional)
                      </Label>
                      <Input 
                        type="password"
              ...
              <CardHeader className="bg-muted/30">
                <CardTitle className="text-lg flex items-center justify-between">
                  ...
                  <div className={`text-xs px-2 py-1 rounded-full ${
                    selectedSubjectIds.length < 5 ? "bg-destructive/10 text-destructive" : "bg-primary/10 text-primary"
                  }`}>
                    {selectedSubjectIds.length} / 10 SELECTED
                  </div>
                </CardTitle>
                <CardDescription>Choose 5-10 categories for the match.</CardDescription>
              </CardHeader>
                    ...
                    {subjectsQuery.data?.map((s: any) => (
                      ...
                      >
                        ...
                        <div className={`absolute bottom-0 left-0 h-1 transition-all ${
                          selectedSubjectIds.includes(s.id) ? "w-full bg-primary" : "w-0 bg-primary/20 group-hover:w-full"
                        }`} />
                      </button>
                    ))}
                  </div>
            ...
            <Card className="bg-muted/50 border-dashed">
              <CardContent className="p-6 flex gap-3">
                ...
                <p className="text-xs text-muted-foreground leading-relaxed">
                  ...
                  Public rooms appear on the global dashboard and are searchable by all players.
                </p>
              </CardContent>
            </Card>
          </div>
